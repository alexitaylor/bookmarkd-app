...
import { genre, bookGenre } from "@bookmarkd/db/schema/genre";
...
async function upsertAuthor(name: string): Promise<{ id: number; name: string }> {
...
async function upsertGenre(name: string): Promise<{ id: number; name: string }> {
	const trimmedName = name.trim();
...
async function getBookAuthors(bookId: number): Promise<{ id: number; name: string }[]> {
...
async function getBookGenres(bookId: number): Promise<{ id: number; name: string }[]> {
	const results = await db
		.select({
			id: genre.id,
			name: genre.name,
